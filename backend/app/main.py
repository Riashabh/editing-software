...
from backend.app.pipeline.extractor import extract_audio
from backend.app.pipeline.analyzer import transcribe_audio, find_best_moments
from backend.app.pipeline.editor import cut_and_merge
from backend.app.pipeline.reframe import reframe_to_vertical
from backend.app.pipeline.subtitles import (
    generate_srt,
...
DEMO_VIDEO_URL = os.getenv("DEMO_VIDEO_URL", "https://pub-270bf75a417c4c969aa92cec22d7d694.r2.dev/demo/demo.mp4")
...
def demo_process(request: Request, job_id: str = "", mode: str = "single", count: int = 1, aspectRatio: str = "original", subtitles: str = "false"):
    ...
    try:
        ...
        req = urllib.request.Request(DEMO_VIDEO_URL, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req) as response, open(input_path, "wb") as f:
...
REMOTION_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "remotion-renderer"))
